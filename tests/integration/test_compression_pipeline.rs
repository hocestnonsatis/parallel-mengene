//! Integration tests for the parallel compression pipeline
...
async fn test_large_file_compression() {
    let temp_dir = tempdir().unwrap();
    let input_path = temp_dir.path().join("large_input.txt");
    let output_path = temp_dir.path().join("large_output.pmz");
    let decompressed_path = temp_dir.path().join("large_decompressed.txt");
...
#[tokio::test]
async fn test_single_byte_file_compression() {
    let temp_dir = tempdir().unwrap();
    let input_path = temp_dir.path().join("single_byte_input.txt");
    let output_path = temp_dir.path().join("single_byte_output.pmz");
    let decompressed_path = temp_dir.path().join("single_byte_decompressed.txt");

    // Create single byte file
    fs::write(&input_path, b"a").unwrap();

    let pipeline = ParallelPipeline::new(CompressionAlgorithm::Gzip).unwrap();
    ...
    // Verify data integrity
    let decompressed_data = fs::read(&decompressed_path).unwrap();
    assert_eq!(decompressed_data, b"a");
}
...
#[tokio::test]
async fn test_decompression_error_handling() {
    let temp_dir = tempdir().unwrap();
    let invalid_input = temp_dir.path().join("invalid.pmz");
    let output_path = temp_dir.path().join("output.txt");

    // Create invalid compressed file
    fs::write(&invalid_input, b"invalid compressed data").unwrap();

    let pipeline = ParallelPipeline::new(CompressionAlgorithm::Lz4).unwrap();
    
    // Try to decompress invalid file
    let result = pipeline.decompress_file(&invalid_input, &output_path).await;
    assert!(result.is_err());
}
...
#[tokio::test]
async fn test_memory_mapping_configuration() {
    let mut pipeline = ParallelPipeline::new(CompressionAlgorithm::Lz4).unwrap();
    
    // Test that we can configure memory mapping
