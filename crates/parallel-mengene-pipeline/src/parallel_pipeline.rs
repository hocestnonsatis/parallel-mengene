...
impl ParallelPipeline {
    ...
    pub async fn compress_file(&self, input_path: &Path, output_path: &Path) -> Result<()> {
        ...
        match strategy {
            CompressionStrategy::SmallFile => {
                ...
                self.cpu_pipeline.compress_file(input_path, output_path).await?;
            }
    ...
    pub async fn decompress_file(&self, input_path: &Path, output_path: &Path) -> Result<()> {
        ...
        match self.coordinator.decompress_chunks(input_path).await {
            Ok(compressed_chunks) => {
                ...
                self.decompress_chunks_parallel(compressed_chunks, output_path).await
            }
            Err(_) => {
                ...
                let decompressed_data = self.cpu_pipeline.decompress_chunk(&compressed_data).await?;
    ...
    async fn decompress_chunks_parallel(&self, compressed_chunks: Vec<Vec<u8>>, output_path: &Path) -> Result<()> {
    ...
    async fn compress_parallel(&self, input_path: &Path, output_path: &Path) -> Result<()> {
        ...
        let optimized_chunk_size = self.memory_tracker.monitor()
        ...
        if optimized_chunk_size != base_chunk_size {
            tracing::info!(
                ...
                base_chunk_size, optimized_chunk_size
            );
        ...
        if self.memory_config.should_use_streaming(file_size) {
            ...
            tracing::info!("Using streaming compression for large file ({} bytes)", file_size);
            ...
            self.compress_with_memory_mapping(input_path, output_path).await
        } else {
    ...
    async fn compress_with_memory_mapping(&self, input_path: &Path, output_path: &Path) -> Result<()> {
        // 1. Create memory-mapped file
        let mmap_file = MemoryMappedFile::new(input_path)?;
        let chunk_size = self.memory_config.calculate_optimal_chunk_size(mmap_file.len());
        ...
        let compression_results: Vec<_> = chunks
            ...
            .map(|(chunk_idx, chunk)| {
                ...
                if let Ok(ref _compressed) = result {
                    ...
                    let mut progress_data = progress_guard.clone();
                    progress_data.update(
                        (chunk_idx + 1) as u64 * chunk_size as u64,
                        chunk_idx + 1,
                    );
                    drop(progress_guard);
        ...
        self.coordinator.combine_chunks(&compressed_chunks, output_path).await?;
        ...
        let compression_results: Vec<_> = chunks
            ...
            .map(|(chunk_idx, chunk)| {
                ...
                if let Ok(ref _compressed) = result {
                    ...
                    let mut progress_data = progress_guard.clone();
                    progress_data.update(
                        (chunk_idx + 1) as u64 * chunk.len() as u64,
                        chunk_idx + 1,
                    );
                    drop(progress_guard);
    ...
    pub fn get_stats(&self) -> PipelineStats {
        PipelineStats {
            ...
            max_workers: self.coordinator.calculate_optimal_workers(1024 * 1024 * 1024), // 1GB example
