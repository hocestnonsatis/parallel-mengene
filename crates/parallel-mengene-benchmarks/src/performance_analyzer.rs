...
impl PerformanceAnalyzer {
    ...
    pub fn analyze_performance(metrics: &[BenchmarkMetrics]) -> Result<PerformanceAnalysis> {
        ...
        let best_overall = algorithm_comparisons
            .first()
            .map(|a| a.algorithm.clone())
            .unwrap_or_default();

        let best_compression_speed = Self::find_best_for_criterion(&algorithm_comparisons, |a| {
            a.compression_speed_stats.mean
        });

        let best_decompression_speed = Self::find_best_for_criterion(&algorithm_comparisons, |a| {
            a.decompression_speed_stats.mean
        });
    ...
    fn analyze_algorithm(
        ...
        let memory_usages: Vec<f64> = metrics
            .iter()
    ...
    fn calculate_statistics(data: &[f64]) -> StatisticalAnalysis {
        ...
        let variance = sorted_data.iter().map(|x| (x - mean).powi(2)).sum::<f64>() / n as f64;
    ...
    fn analyze_scaling_performance(
        metrics: &[BenchmarkMetrics],
    ) -> Result<HashMap<String, ScalingAnalysis>> {
        ...
        for metric in metrics {
            ...
            size_groups.entry(size_category).or_default().push(metric);
        }
        ...
        for (size_category, size_metrics) in size_groups {
            ...
            let best_algorithm = algorithms_performance
                .iter()
    ...
    fn generate_recommendations(comparisons: &[AlgorithmComparison]) -> Vec<String> {
        ...
        let fastest_compression = comparisons
            .iter()
            .max_by(|a, b| {
                a.compression_speed_stats
                    .mean
                    .partial_cmp(&b.compression_speed_stats.mean)
                    .unwrap_or(std::cmp::Ordering::Equal)
            })
            .unwrap();
        ...
        recommendations.push(format!(
            ...
            fastest_compression.algorithm, fastest_compression.compression_speed_stats.mean
        ));

        // Memory recommendations
        let most_memory_efficient = comparisons
            .iter()
            .min_by(|a, b| {
                a.memory_usage_stats
                    .mean
                    .partial_cmp(&b.memory_usage_stats.mean)
                    .unwrap_or(std::cmp::Ordering::Equal)
            })
            .unwrap();
        ...
        recommendations.push(format!(
            ...
            most_memory_efficient.algorithm, most_memory_efficient.memory_usage_stats.mean
        ));

        // Reliability recommendations
        let most_reliable = comparisons
            .iter()
            .max_by(|a, b| {
                a.reliability_score
        ...
        recommendations.push(format!(
            ...
            most_reliable.algorithm, most_reliable.reliability_score
        ));
...
mod tests {
    ...
    fn test_file_size_categorization() {
        assert_eq!(
            PerformanceAnalyzer::categorize_file_size(500 * 1024),
            "Small (< 1MB)"
        );
        assert_eq!(
            PerformanceAnalyzer::categorize_file_size(5 * 1024 * 1024),
            "Medium (1-10MB)"
        );
        assert_eq!(
            PerformanceAnalyzer::categorize_file_size(50 * 1024 * 1024),
            "Large (10-100MB)"
        );
        assert_eq!(
            PerformanceAnalyzer::categorize_file_size(200 * 1024 * 1024),
            "Very Large (> 100MB)"
        );
    }
}
