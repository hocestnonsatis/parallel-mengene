...
use crate::metrics_collector::{BenchmarkMetrics, MetricsCollector, measure_operation, OperationMetrics};
...
impl BenchmarkRunner {
    ...
    async fn run_single_file_benchmark(
        ...
        for i in 0..self.config.warmup_iterations {
            info!("🔥 Warmup iteration {}/{}", i + 1, self.config.warmup_iterations);
            let _ = self.run_single_iteration(algorithm, test_file, file_size).await;
        }
        
        // Run actual benchmark iterations
        for i in 0..self.config.iterations {
            info!("🔄 Benchmark iteration {}/{}", i + 1, self.config.iterations);
            
            let metrics = self.run_single_iteration(algorithm, test_file, file_size).await?;
    ...
    async fn run_single_iteration(
        ...
        let compressed_file = temp_dir.join(format!("compressed_{}.pmz", 
            test_file.file_name().unwrap().to_string_lossy()));
        let decompressed_file = temp_dir.join(format!("decompressed_{}", 
            test_file.file_name().unwrap().to_string_lossy()));
        ...
        // Verify integrity
        let integrity_check = self.verify_file_integrity(test_file, &decompressed_file).await?;
    ...
    pub fn generate_summary(&self, metrics: &[BenchmarkMetrics]) -> BenchmarkSummary {
        ...
        let peak_memory_usage = metrics.iter()
            .map(|m| m.compression_metrics.memory_peak_mb)
            .fold(0.0, f64::max);
        
        let peak_cpu_usage = metrics.iter()
